...
def index():
    print("the number of topics are: " + str(NUM_TOPICS))
    if "knowledge" not in session:
        session["knowledge"] = [0.1] * NUM_TOPICS
    elif len(session["knowledge"]) < NUM_TOPICS:
        session["knowledge"].extend([0.1] * (NUM_TOPICS - len(session["knowledge"])))

    state = np.array(session["knowledge"])
    action = agent.act(state, prev_action=session.get("prev_action"))
...
def submit():
    ...
    topic_scores = list(enumerate(session["knowledge"]))
    return render_template("feedback.html", feedback=feedback, topic_scores=topic_scores)
