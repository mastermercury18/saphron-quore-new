...
class DQNAgent:
    ...
    # Step 1: Grover-based probabilistic sampling of good actions
def grover_sample_actions(q_values, num_samples=3):
    ...
    # Get the most frequent measurement results
    sorted_bins = sorted(counts.items(), key=lambda x: x[1], reverse=True)
    sampled_actions = [int(bin_str, 2) for bin_str, _ in sorted_bins]
    sampled_actions = [a for a in sampled_actions if a < n_actions]
    return sampled_actions[:num_samples]


# Step 2: QAOA selection with constraints from sampled actions
def qaoa_filter_from_candidates(q_values, topics, difficulties, candidate_indices,
                                prev_topic=None, max_difficulty=2):
    mdl = Model()
    x = mdl.binary_var_list(len(candidate_indices), name='x')

    # Constraint: only one question selected
    mdl.add_constraint(mdl.sum(x) == 1)

    # Constraint: avoid previous topic
    if prev_topic is not None:
        for i, idx in enumerate(candidate_indices):
            if topics[idx] == prev_topic:
                mdl.add_constraint(x[i] == 0)

    # Constraint: respect max difficulty
    for i, idx in enumerate(candidate_indices):
        if difficulties[idx] > max_difficulty:
            mdl.add_constraint(x[i] == 0)

    # Objective: maximize Q-values (i.e., minimize -Q)
    mdl.minimize(-mdl.sum(q_values[candidate_indices[i]] * x[i] for i in range(len(candidate_indices))))
    ...
    return candidate_indices[selected[0]] if selected else candidate_indices[np.argmax([q_values[i] for i in candidate_indices])]


# 🔚 Final Combined Function
def hybrid_grover_qaoa_action_selector(q_values, topics, difficulties,
                                    prev_topic=None, max_difficulty=2,
                                    num_samples=4):
    candidates = grover_sample_actions(q_values, num_samples=num_samples)
    return qaoa_filter_from_candidates(
        q_values,
        topics,
        difficulties,
        candidates,
